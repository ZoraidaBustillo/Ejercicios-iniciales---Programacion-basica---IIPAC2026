// ZORAIDA VERENICE BUSTILLO MARADIAGA //
// CUENTA: 20191006821// 
...
// ================= ESTADÍSTICAS (NUEVO) =================
int ventasProducto[NUM_PRODUCTOS] = {0};
int pagoEfectivo = 0;
int pagoTransferencia = 0;
double gananciaCategoria[4] = {0};
...
void factura(string nombre, int edad, char genero, int tipo,
    ...
    for (int i = 0; i < NUM_PRODUCTOS; i++) {
        if (cant[i] > 0) {
            ...
            if (categoriasProductos[i] == "Celulares")
                gananciaCategoria[0] += preciosProductos[i] * cant[i];

            if (categoriasProductos[i] == "Laptops")
                gananciaCategoria[1] += preciosProductos[i] * cant[i];

            if (categoriasProductos[i] == "Tablets")
                gananciaCategoria[2] += preciosProductos[i] * cant[i];

            if (categoriasProductos[i] == "Accesorios")
                gananciaCategoria[3] += preciosProductos[i] * cant[i];
...
int main() {
    ...
    do {
        ...
        if (pago == 1) {
            ...
            pagoTransferencia++; // NUEVO

            string referencia, bancoOrigen, bancoDestino;
            double monto;
            ...
            cout << "Banco de emision: ";
            cin >> bancoOrigen;

            cout << "Banco de destino: ";
            cin >> bancoDestino;

            cout << "Ingrese monto exacto (Lps): ";
            cin >> monto;

            while (monto != total) {
                cout << "El monto debe ser EXACTO: Lps " << total << "\n";
                cout << "Ingrese nuevamente: ";
                cin >> monto;
            }

            cout << "Transferencia registrada correctamente.\n";
        }
    ...
    for (int i = 1; i < NUM_PRODUCTOS; i++) {
        if (ventasProducto[i] > ventasProducto[maxI]) maxI = i;
        if (ventasProducto[i] < ventasProducto[minI]) minI = i;
    }

    cout << "\n--- ESTADISTICAS ---\n";
    cout << "Mas vendido: " << nombresProductos[maxI] << "\n";
    cout << "Menos vendido: " << nombresProductos[minI] << "\n";

    cout << "Pago efectivo: " << pagoEfectivo << "\n";
    cout << "Pago transferencia: " << pagoTransferencia << "\n";

    cout << "\n--- GANANCIA POR CATEGORIA ---\n";
    cout << "Celulares: Lps " << gananciaCategoria[0] << "\n";
    cout << "Laptops: Lps " << gananciaCategoria[1] << "\n";
    cout << "Tablets: Lps " << gananciaCategoria[2] << "\n";
    cout << "Accesorios: Lps " << gananciaCategoria[3] << "\n";
